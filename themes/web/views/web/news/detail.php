...
<div id="ctl27_ctl01_panelNewsDetails" class="contentDetail">
    ...
    <h1 id="ctl27_ctl01_divArticleTitle" class="detailsView-title-style"><?php echo $news->title?></h1>
    <div id="ctl27_ctl01_divDate" class="date-first"><?php echo date("d/m/Y H:i", $news->created)?></div>
        ...
        <?php if(count($topic)>0):?>
            <div>
                <div class="inasub">
                    ...
                    <a href="<?php echo Yii::app()->createUrl('/web/news/topic', array('alias'=>$news->topic->alias,'id' => $news->topic_id))?>"><?php echo $news->topic->title?></a></div>
                <div class="subinart">
                    <?php foreach($topic as $_key_topic => $_val_topic):?>
                        <div class="artlist">
                            <a href="<?php echo Yii::app()->createUrl('/web/news/detail', array('alias'=>$_val_topic['alias'],'id' => $_val_topic['id']))?>" class="line"><?php echo $_val_topic['title']?></a>
                        </div>
                    <?php endforeach;?>
                </div>
            </div>
    ...
    <div class="detailsView-contents-style">
        <h2 id="ctl27_ctl01_divSummary" class="summary"><?php echo $news->desc?></h2>

    </div>
    <div id="divContents" class="detailsView-contents-style detail-article-content" style="overflow: hidden;"><?php echo $news->content?></div>
    ...
    <div class="othernews">
        <?php if(count($same) > 0):?>
            ...
            <?php foreach($same as $_k_news => $_val_news):?>
                <a class="font-link-box-item iconlist" href="<?php echo Yii::app()->createUrl('/web/news/detail', array('alias'=>$_val_news['alias'],'id' => $_val_news['id']))?>" title="<?php echo $_val_news['title']?>"><?php echo $_val_news['title']?><i>(<?php echo date("d/m", $_val_news['created'])?>)</i></a>
            <?php endforeach;?>
        <?php endif;?>
    </div>
...
<div style="text-align: right;">
    <a id="lnkNewOther" class="normalblue" href="<?php echo Yii::app()->createUrl('/web/news/list', array('alias'=>$news->aliasTypeLabel))?>" style="cursor: pointer;
