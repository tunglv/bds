...
class SaleController extends AdminController {
    ...
    public function init() {
        parent::init();
        $this->layout = '//admin/sale/_layout';
        $this->menu_parent_selected = 'sale';
    }
...
//            $criteria->order = 't.id ASC';
    ...
    public function actionCreate() {
        $this->menu_child_selected = 'sale_create';
        ...
        if (isset($_POST['BdsSale'])) {
            $post = Yii::app()->request->getPost('BdsSale');
...
//                        }
                ...
                if(Yii::app()->session['productImgTemp']){
                    foreach(Yii::app()->session['productImgTemp'] as $imgPath){
                        ...
                        //tạo thumb
                        $fileName = ImagesSale::model()->createThumb($imgPath, $savedPath,$imgConf);

//                        unlink($imgPath);
                        //insert vào bảng branch
                        $branchImage = new ImagesSale();
                        $branchImage->bds_sale_id = $model->id;
...
//                }
    ...
    public function actionUpdate($id) {
        $this->menu_child_selected = 'sale_update';
        $this->menu_sub_selected = 'update';

        $model = BdsSale::model()->findByPk($id);
        ...
        if (isset($_POST['BdsSale'])) {
            ...
            if ($model->validate()) {
                ...
                // detect image cover
                if(!$coverImg = Yii::app()->request->getPost('is_cover')){
                    $branchImage  = ImagesSale::model()->findByAttributes(array('bds_sale_id' => $model->id));
                    if($branchImage)
                ...
                ImagesSale::model()->updateAll(array('is_cover' => 0), "bds_sale_id = {$model->id}");
                ImagesSale::model()->updateAll(array('is_cover' => 1), "bds_sale_id = {$model->id} AND image = '{$coverImgName}'");
    ...
    public function actionDelete($id) {
        $product = BdsSale::model()->findByPk($id);
    ...
    private function _uploadGetImages() {
        ...
        if($id){
            $images = ImagesSale::model()->findAllByAttributes(array('bds_sale_id' => $id));
            $data = array();
            $name_cookie = '';
            foreach($images as $img){
                ...
                $data['files'][] = array(
                    ...
                    'delete_url' => $this->createAbsoluteUrl("/admin/sale/uploadImages", array('id' => $id, 'product_image_id'=>$img->id)),
                    'delete_type' => 'DELETE',
                );
            }
            Yii::app()->request->cookies['product_form_img_cover'] = new CHttpCookie('product_form_img_cover', $name_cookie.'.jpg', array('path' => '/admin/sale/'));
                ...
                foreach($productImgTemp as $path){
                    if(file_exists($path)){
                        ...
                        $fileData = array(
                            ...
                            'delete_url' => $this->createAbsoluteUrl("/admin/sale/uploadImages", array('path' => $path, 'id' => $id)),
                            'delete_type' => 'DELETE',
                        );
                        $data['files'][] = $fileData;
                    }
...
//            $product = Branch::model()->with('shop')->findByPk($id);
            ...
            ImagesSale::model()->createThumb($_FILES["files"]["tmp_name"][0], $savedPath, $imgConf, $fileName);

            // insert to product_image
            $productImage = new ImagesSale();
            $productImage->bds_sale_id = $id;
            ...
            $data = array(
                'files' => array(
                    array(
                        ...
                        'delete_url' => $this->createAbsoluteUrl("/admin/sale/uploadImages", array('id' => $id, 'product_image_id' => $productImage->id)),
                        'delete_type' => 'DELETE',
                    )
                )
            );
            ...
            $data = array(
                'files' => array(
                    array(
                        ...
                        'delete_url' => $this->createAbsoluteUrl("/admin/sale/uploadImages", array('path' => $tempPath.$fileName, 'id' => $id)),
                        'delete_type' => 'DELETE',
                    )
                )
            );
    ...
    private function _uploadDeleteImages(){
        ...
        if($product_image_id){
            $img = ImagesSale::model()->findByPk($product_image_id);
            $img->delete();
